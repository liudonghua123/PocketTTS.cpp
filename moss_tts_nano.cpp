// MOSS-TTS-Nano.cpp — Single-file C++ TTS runtime using ONNX Runtime
// Based on PocketTTS.cpp, adapted for MOSS-TTS-Nano-100M and MOSS-Audio-Tokenizer-Nano
...
public:
    ...
    explicit MossTTSNano(const Config& cfg = {}) : cfg_(cfg) {
        ...
        enc_ = std::make_unique<OrtSession>(env, cfg_.models_dir + "/audio_encoder.onnx", opts_enc, "audio_encoder");
        txt_ = std::make_unique<OrtSession>(env, cfg_.models_dir + "/text_encoder.onnx", opts_full, "text_encoder");
        main_ = std::make_unique<OrtSession>(env, cfg_.models_dir + "/flow_model" + sfx + ".onnx", opts_ar, "flow_model" + sfx);
        flow_ = std::make_unique<OrtSession>(env, cfg_.models_dir + "/flow_model" + sfx + ".onnx", opts_ar, "flow_model" + sfx);  // MOSS may combine these
        dec_ = std::make_unique<OrtSession>(env, cfg_.models_dir + "/audio_decoder" + sfx + ".onnx", opts_dec, "audio_decoder" + sfx);
...
private:
    ...
    public:
        ...
        LatentGen(MossTTSNano& t, const Tensor& v, const TensorI64& tid, int max, int eos_extra, Snapshot* out_voice_snap = nullptr)
        ...
        LatentGen(MossTTSNano& t, const Snapshot& voice_snap, const TensorI64& tid, int max, int eos_extra)
            : tts(t), max_(max), eos_extra_(eos_extra), m_(Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault)),
              fx_(32, 0), cl_(32, std::numeric_limits<float>::quiet_NaN()),
              main_runner_(*tts.main_runner_) {
            temp_ = std::sqrt(tts.cfg_.temperature);
...
AudioData MossTTSNano::generate(const std::string& text, const Tensor& voice, int max_frames) {
...
void MossTTSNano::stream(const std::string& text, const std::string& voice, StreamCallback cb, int max_frames) {
    stream(text, get_voice(voice), cb, max_frames);
}

void MossTTSNano::stream(const std::string& text, const Tensor& voice, StreamCallback cb, int max_frames) {
    auto sentences = split_sentences(text);
    if (sentences.empty()) sentences.push_back(text);
...
public:
    TTSServer(MossTTSNano& tts, int port) : tts_(tts), port_(port) {}
...
void* ptt_create(const char* models_dir, const char* voices_dir,
    ...
    try {
        ...
        return new moss_tts_nano::MossTTSNano(cfg);
    } catch (const std::exception& e) {
...
double ptt_warmup(void* handle) {
    if (!handle) return -1;
    try {
        return static_cast<moss_tts_nano::MossTTSNano*>(handle)->warmup();
    } catch (const std::exception& e) {
...
void ptt_destroy(void* handle) {
    delete static_cast<moss_tts_nano::MossTTSNano*>(handle);
}
...
void* ptt_stream_start(void* handle, const char* text, const char* voice) {
    ...
    auto* tts = static_cast<moss_tts_nano::MossTTSNano*>(handle);
...
static void signal_handler(int sig) {
    (void)sig;
    moss_tts_nano::g_server_running = false;
    if (moss_tts_nano::g_server_fd != PTT_INVALID_SOCKET) {
        ptt_close(moss_tts_nano::g_server_fd);
        moss_tts_nano::g_server_fd = PTT_INVALID_SOCKET;
    }
    std::cout << "\nShutting down...\n";
}
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "--profile") moss_tts_nano::g_prof.enabled = true;
    ...
    try {
        ...
        if (server_mode) {
            ...
            if (stdout_output) {
                ...
                audio.sample_rate = moss_tts_nano::MossTTSNano::SR;
                ...
                if (pocket_tts::g_prof.enabled) {
                    ...
                    audio = {std::move(samples), moss_tts_nano::MossTTSNano::SR};
                } else {
            ...
            if (!stdout_output) {
                ...
                moss_tts_nano::MossTTSNano::save_audio(audio, output);
